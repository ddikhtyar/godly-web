...
module.exports = {
  ...
  viewProfile: function(req, res) {
    // First, look up the requesting user's info, pre-formatted.
    // (e.g. so we can use it in our topbar)
    User.fetchForTopbar(req, function (err, requestingUserInfo){
      if (err && err.code === 'E_ORPHANED_SESSION') {
        req.session.userId = null;
        return res.redirect('/login');
      }
      if (err) { return res.serverError(err); }

      // Now we need to look up the user record whose profile
      // we're actually looking at:
      User.findOne({
        id: req.param('id')
      })
      .populate('wallPosts', { limit: 15, sort: 'createdAt DESC' })
      .exec(function(err, profileUser) {
        if(err) { return res.serverError(err); }
        if(!profileUser) { return res.notFound(); }

        // Finally, IWMIH, we can respond with the actual webpage.
        return res.view('pages/profile', {

          // e.g. for the HTML `<title>` tag
          pageTitle: profileUser.firstName+'\'s profile',

          // e.g. for the main content of the page
          profileData: _.omit(profileUser, 'password'),

          // e.g. for the top-right corner of the screen
          me: requestingUserInfo
